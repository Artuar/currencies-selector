...
import { allCurrenciesSelector, chosenCurrenciesSelector } from "../../store/currencies/currencies.selector";
...
const useDispatchActions = () => {
  ...
  return {
    choose: (currency: string) =>
      dispatch(currenciesActions.choose(currency)),
...
export const CurrenciesList: React.FunctionComponent = () => {
  ...
  const handleCkick = (currency: string, chosen: boolean) => chosen ? remove(currency) : choose(currency)
    ...
    <article id="currencies-list" className={styles.allCurrencies}>
      {
        allCurrencies.map(currency => {
          ...
          return <button key={`choose-${currency}`} id={`choose-${currency}`} onClick={() => handleCkick(currency, chosen)} className={
            classNames(styles.chooseCurrency, {[styles.chosen]: chosen})}>
